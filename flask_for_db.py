...
@app.route('/api/get_frame/<int:frame_id>', methods=['GET'])
def get_frame(frame_id):
    c.execute("SELECT frame FROM tespit WHERE id = %s", (frame_id,))
    record = c.fetchone()
    frame_data = record[0]
    return send_image(frame_data)
...
# Görüntüyü alma ve işleme endpoint'i
...
# Belirli bir kareyi alma endpoint'i
@app.route('/api/get_frame/<int:frame_id>', methods=['GET'])
def get_frame(frame_id):
    c.execute("SELECT frame FROM tespit WHERE id = %s", (frame_id,))
    record = c.fetchone()
